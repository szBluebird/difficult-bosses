function OnStartTouch(trigger)
    ...
    -- Trigger particles
    if GameMode.bEnigmaBossInProgress and (not GameMode.bEnigmaBossRightPillarActive) then

        -- Trigger the particles on the outer piller
        local hOuterStartRelay = Entities:FindByName( nil, "enigma_boss_outer_right_pillar_relay_start")
        if hOuterStartRelay ~= nil then
            hOuterStartRelay:Trigger()
        end
        GameMode.bEnigmaBossRightPillarActive = true

        -- Trigger the particles on the inner piller
        local hInnerStartRelay = Entities:FindByName( nil, "enigma_boss_inner_right_pillar_relay_start")
        if hInnerStartRelay ~= nil then
            hInnerStartRelay:Trigger()
        end

        -- Activate the inner pillar
        local hInnerPillar = Entities:FindByName(nil, "enigma_boss_inner_void_pillar_right")
